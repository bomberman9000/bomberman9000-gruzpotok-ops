...
# 3. gruzpotok-api (host port 8002 -> container 8000)
API_STATUS=$(curl -s --max-time 5 http://localhost:8002/health | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('status','?'))" 2>/dev/null || echo "недоступен")
check "gruzpotok-api /health" "$([ "$API_STATUS" = "ok" ] || [ "$API_STATUS" = "healthy" ] && echo 1 || echo 0)" "$API_STATUS"

# 4. tg-bot (host port 18091 -> container 8000; port 8000 used by PAPPL)
BOT_STATUS=$(curl -s --max-time 5 http://localhost:18091/health | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('status','?'))" 2>/dev/null || echo "недоступен")
check "tg-bot /health" "$([ "$BOT_STATUS" = "healthy" ] || [ "$BOT_STATUS" = "degraded" ] && echo 1 || echo 0)" "$BOT_STATUS"
...
# 6. Storage1 доступен (проверяем backup dir, а не mountpoint — путь может быть внутри FS)
