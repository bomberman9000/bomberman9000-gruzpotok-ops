...
_ROOT = _SCRIPT_DIR.parents[0]
if str(_ROOT / "rag-service") not in sys.path:
...
def _parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Извлечь текст из PDF, прогнать через Ollama и вернуть summary/facts/draft reply."
    )
    p.add_argument("--file", required=True, help="Путь к PDF-файлу")
    p.add_argument(
        "--task",
